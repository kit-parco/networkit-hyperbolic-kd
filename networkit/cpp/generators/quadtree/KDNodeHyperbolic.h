...
class KDNodeHyperbolic: public NetworKit::SpatialCell<T> {
public:
	KDNodeHyperbolic() = default;
	virtual ~KDNodeHyperbolic() = default;
	...
	void split() override {
		...
		std::shared_ptr<KDNodeHyperbolic<T, poincare> > firstChild(new KDNodeHyperbolic<T, poincare>(this->minCoords, newUpper, this->capacity));
		std::shared_ptr<KDNodeHyperbolic<T, poincare> > secondChild(new KDNodeHyperbolic<T, poincare>(newLower, this->maxCoords, this->capacity));
